...
export function HippoSpeechBubble({
  ...
  return (
    <AnimatePresence mode="wait">
      <motion.div
        key={text || "typing"}
        initial={{ opacity: 0, scale: 0.8, y: 18, rotate: -1.5 }}
        animate={{ opacity: 1, scale: 1, y: 0, rotate: 0 }}
        exit={{ opacity: 0, scale: 0.9, y: -10, rotate: 1 }}
        transition={{ type: "spring", damping: 16, stiffness: 300 }}
        className={cn(
          "relative max-w-sm rounded-[28px] border border-primary/20 bg-card/95 p-4 shadow-xl backdrop-blur-md dark:border-primary/30",
          className
        )}
      >
